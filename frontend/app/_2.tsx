...
import { View, Text, StyleSheet, ActivityIndicator, Image, TextInput, TouchableOpacity, Platform } from 'react-native';
...
const UpdateEmployeeScreen = ({ route }: { route: any }) => {
    ...
    const handleImageSelect = async () => {
        ...
        if (!result.canceled && result.assets && result.assets.length > 0) {
            ...
            try {
                // Use the URI directly, conversion will happen in the upload function
                await uploadProfilePicture(employeeId, selectedImage.uri);
